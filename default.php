            ...
            <?php
                ...
                if($_SERVER['REQUEST_METHOD'] === 'POST'){
                    ...
                    if(empty($email)){
                        ...
                        $sql = "SELECT * FROM sk_admin WHERE admin_email = ?";
                        ...
                        if($stmt->execute()){
                            ...
                            if($result->num_rows > 0){
                                ...
                                if($user['isVerified'] === 1){
                                    if(password_verify($password, $user['admin_password']) || $password === $user['admin_password']){
                                        echo "<p>Login successful! Welcome, " . htmlspecialchars($user['admin_email']) . "</p>";
                                            $_SESSION['name'] = $user['admin_name'];
                                            $_SESSION['adminId'] = $user['admin_id'];
                                            $_SESSION['position'] = $user['admin_position'];
